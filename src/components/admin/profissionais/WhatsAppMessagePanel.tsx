...
const WhatsAppMessagePanel = ({
  ...
  const sendWhatsappToFiltered = async () => {
    ...
    try {
      ...
      const telefones = filteredProfissionais.map(p => p.whatsapp).filter(whatsapp => whatsapp && whatsapp.trim() !== '').map(whatsapp => addBrazilCode(whatsapp));
  ...
  return <div className="items-center max-w-full items-center ">
      ...
      {whatsappButtonState === 'ready' && <div className="p-4 border rounded-lg bg-green-50 space-y-3">
          ...
          <Textarea placeholder="Digite sua mensagem..." value={whatsappMessage} onChange={e => setWhatsappMessage(e.target.value)} className="w-full min-h-[120px] max-h-[300px] resize-y" rows={5} style={{
        height: 'auto',
        minHeight: '120px'
      }} />
