...
const FormularioDemanda = ({
  ...
  useEffect(() => {
    if (selectedCategoryId && form.getValues('subcategoria_id') && !filteredSubcategorias.find(sub => sub.id === form.getValues('subcategoria_id'))) {
  ...
  return <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField control={form.control} name="nome" rules={{
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <FormField control={form.control} name="whatsapp" rules={{
          ...
          field
                ...
                <FormControl>
                  <Input placeholder="(00) 00000-0000" value={field.value || ''} onChange={e => {
          ...
          <FormField control={form.control} name="estado" rules={{
          ...
          field
                ...
                <Select onValueChange={field.onChange} value={field.value}>
                  ...
                  <SelectContent className="bg-white max-h-60 overflow-y-auto">
                    {estadosBrasil.map(estado => <SelectItem key={estado.sigla} value={estado.sigla}>
          ...
          <FormField control={form.control} name="cidade" rules={{
          ...
          field
                ...
                <Select onValueChange={field.onChange} value={field.value} disabled={!selectedEstado || loadingCidades}>
                  ...
                  <SelectContent className="bg-white max-h-60 overflow-y-auto">
                    {cidades.map(cidade => <SelectItem key={cidade.id} value={cidade.nome}>
                        {cidade.nome}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <FormField control={form.control} name="categoria_id" rules={{
          ...
          field
                ...
                return <SelectItem key={categoria.id} value={categoria.id}>
          ...
          <FormField control={form.control} name="subcategoria_id" rules={{
          ...
          field
                ...
                <Select onValueChange={field.onChange} value={field.value} disabled={!selectedCategoryId}>
                  ...
                  <SelectContent className="bg-white max-h-60 overflow-y-auto">
                    {filteredSubcategorias.map(subcategoria => <SelectItem key={subcategoria.id} value={subcategoria.id}>
                        {subcategoria.nome}
                      </SelectItem>)}
                  </SelectContent>
                </Select>
        ...
        {/* Urgência */}
        <FormField control={form.control} name="urgencia" rules={{
        required: 'Urgência é obrigatória'
      }} render={({
        field
      }) => <FormItem>
        ...
        {/* Observação */}
        <FormField control={form.control} name="observacao" render={({
        field
      }) => <FormItem>
              <FormLabel>Observação da solicitação</FormLabel>
              <FormControl>
                <Textarea placeholder="Descreva detalhes da sua solicitação..." className="min-h-[100px]" {...field} />
              </FormControl>
              <FormMessage />
            </FormItem>} />
      ...
      }} disabled={submitting} className="w-full h-12 text-lg font-semibold rounded-full">
          {submitting ? <>
